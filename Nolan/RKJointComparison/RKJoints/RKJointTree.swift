...
class RKJointTree: Codable {
    ...
    func isEquivalent(to other: RKImmutableJointTree) -> Bool {
        guard let rootJoint = self.rootJoint, let otherRootJoint = other.rootJoint else {
            return false
        }
    ...
    func score(to otherTree: RKImmutableJointTree, consideringJoints jointList: [String]) -> (Float, Float, Float, Float) { //Min, Max, Average, Median
        ...
        let totalSum = squaredDistances.reduce(0, { $0 + $1})
        let avg = totalSum / Float(squaredDistances.count > 0 ? squaredDistances.count : 1)
